...
def internal_cost(G, labels, vertex):
    internal_cost = 0
    V = G.nodes()
    E = G.edges(data=True)
    ...
    for label in labels:
        if label == vertex_label:
            ...
            if(len(common_edge) > 0):
                ...
                internal_cost += internal_cost_to_add
        neighbor_index += 1
    
    return internal_cost

def external_cost(G, labels, vertex):
    external_cost = 0
    V = G.nodes()
    E = G.edges(data=True)
    vertex_index = V.index(vertex)
    ...
    for label in labels:
        if label != vertex_label:
            ...
            if(len(common_edge) > 0):
                ...
                external_cost += external_cost_to_add
        neighbor_index += 1
...
labels = [-1, 1, -1, -1, 1, 1]
partition_cut = cut(G, labels)
print("final cut: " + str(partition_cut))
V = G.nodes()
vertex = V[4]
internal_cost_one = internal_cost(G, labels, vertex)
print("final internal cost of the node " + str(vertex) + ": " +
      str(internal_cost_one))
external_cost_test = external_cost(G, labels, vertex)
